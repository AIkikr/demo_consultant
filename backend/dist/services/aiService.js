...
const webSearchService_1 = require("./webSearchService");
const uuid_1 = require("uuid");
class AIService {
    constructor() {
        this.webSearchService = webSearchService_1.WebSearchService.getInstance();
    }
    static getInstance() {
     ...
     * AIレスポンスを生成
     */
    async generateResponse(message, mode, sessionId, messageHistory = []) {
        try {
     ...
     */
    async processKnowledgeSteps(message, mode) {
        // Step A: 既存知識での暫定考察
    ...
    generateStepAResponse(message, mode) {
        const modeConfig = constants_1.MODE_PROMPTS[mode];
        // モードに応じた基本的なレスポンス生成
        switch (mode) {
            case 'guide':
...
厳しい質問ですが、これらに明確に答えられなければ、成功は困難かもしれません。`;
    ...
    generateStepCResponse(message, mode, stepA, stepB) {
